...
@Component({
	selector: 'app-zona-status-chart',
	standalone: true,
	imports: [CommonModule, MatIconModule],
	templateUrl: './zona-status-chart.component.html',
...
export class ZonaStatusChartComponent implements OnChanges, AfterViewInit, OnDestroy {
	...
	private generateStatusColors(statuses: string[]): string[] {
		const colorMap: { [key: string]: string } = {
			'Disponible': 'rgba(16, 185, 129, 0.8)', // Verde
			'En Inventario': 'rgba(59, 130, 246, 0.8)', // Azul
			'En Verificación': 'rgba(245, 158, 11, 0.8)' // Amarillo
		};

		return statuses.map(
			(status) => colorMap[status] || 'rgba(156, 163, 175, 0.8)'
		);
	}

	private generateStatusHoverColors(statuses: string[]): string[] {
		const colorMap: { [key: string]: string } = {
			'Disponible': 'rgba(16, 185, 129, 1)',
			'En Inventario': 'rgba(59, 130, 246, 1)',
			'En Verificación': 'rgba(245, 158, 11, 1)'
		};

		return statuses.map(
			(status) => colorMap[status] || 'rgba(156, 163, 175, 1)'
		);
	}
}
